...
// Constructor
ModelInferenceONNX::ModelInferenceONNX(const char* model_path) 
...
// Destructor
ModelInferenceONNX::~ModelInferenceONNX() {
    // Any necessary cleanup (optional since ONNX Runtime automatically manages memory)
}

// Method to perform inference
std::vector<Ort::Value> ModelInferenceONNX::RunInference(const std::vector<std::vector<float>>& input_data) {
...
void ModelInferenceONNX::PrintModelInfo() const {
    for (size_t i = 0; i < num_input_nodes; ++i) {
